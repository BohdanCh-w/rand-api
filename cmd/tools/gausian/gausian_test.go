...
func TestGausianCommand_SuccessNoParam(t *testing.T) {
	...
	gomock.InOrder(
		...
		mockRandRetriever.EXPECT().
			...
			Return(helpers_test.TestRandResult(t, "[-0.244817]"), nil),

		mockOutputProcessor.EXPECT().
			GenerateRandOutput([]any{-0.244817}, helpers_test.TestRandAPIInfo(t, req.ID)).
			Return(nil),
	)
...
func TestGausianCommand_SuccessWithParams(t *testing.T) {
	...
	gomock.InOrder(
		...
		mockRandRetriever.EXPECT().
			ExecuteRequest(gomock.Any(), &req).
			Return(helpers_test.TestRandResult(t, "[0.224795, 0.582253, -0.580953]"), nil),

		mockOutputProcessor.EXPECT().
			GenerateRandOutput([]any{0.224795, 0.582253, -0.580953}, helpers_test.TestRandAPIInfo(t, req.ID)).
			Return(nil),
	)
